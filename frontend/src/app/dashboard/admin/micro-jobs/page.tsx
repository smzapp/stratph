...
export default function AdminMicroJobModeration() {
  const { db, settings, adminModerateMicroJob, updateSettings } = useApp();
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-zinc-500">
              {settings?.microJobAutoApprove
                ...
                : "New postings stay pending until a Super Admin approves them."}
            </p>
          </div>
          <button
            onClick={() => updateSettings({ microJobAutoApprove: !settings?.microJobAutoApprove })}
            className={`relative h-7 w-12 shrink-0 rounded-full transition-colors ${
              settings?.microJobAutoApprove ? "bg-indigo-600" : "bg-zinc-300"
            }`}
            aria-pressed={!!settings?.microJobAutoApprove}
          >
            <span
              className={`absolute top-1 h-5 w-5 rounded-full bg-white transition-transform ${
                settings?.microJobAutoApprove ? "translate-x-6" : "translate-x-1"
              }`}
            />
          </button>
        </div>
